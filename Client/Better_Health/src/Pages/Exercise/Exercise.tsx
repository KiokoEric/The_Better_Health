...
const Exercise: React.FC = () => {
    ...
    const SearchExercise = async (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault()
...
return (
    <div>
        <section id='Exercise' className='flex flex-col items-center justify-center gap-5 mb-10 text-white'>
            <h1 className='font-bold text-5xl'>Search your body part</h1>
            <form onSubmit={SearchExercise} className="bg-white flex flex-row items-center justify-between gap-1 px-1 py-1 rounded-sm w-2/5">
                <FontAwesomeIcon icon={faDumbbell} width="2rem" color="black"  />
                <select name="Select" id="Select" className='h-10 outline-none px-2 py-1 text-black w-5/6' value={Search} onChange={handleSearch} >
